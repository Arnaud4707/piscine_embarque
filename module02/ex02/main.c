...
void uart_init(void)
{
    ...
    // UCSZ01 (bit 2) et UCSZ00 (bit 1) : ensemble definissent la taille des donnees. 1 octe
...
uint8_t uart_rx()
{
    ...
    // RXC0 (bit 5) : USART Data Register Empty → vaut 1 quand le registre RXC0 est vide pour recevoir le prochain octet
    while (!(UCSR0A & (1 << RXC0)));  // Attente buffer vide
    return UDR0;
}
