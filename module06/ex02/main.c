...
void convert_dizaine(float d)
{
    uint8_t counter_digit = 0;
    uint16_t deci = (d * 100) + 0.5;
    uint16_t v = (uint16_t)deci % 100;
...
void AHT20_read(float *temperature, float *humidity)
{
    ...
    for (uint8_t i = 0; i < 5; i++)
        data[i] = i2c_read_ack(); // lire 5 octets avec ACK

    data[5] = i2c_read_nack(); // dernier octet avec NACK
    i2c_stop();

    status = data[0];
    if (status & 0x80) {
        // capteur encore occupé
        *temperature = -999.0;
        *humidity = -1.0;
        return;
    }
...
int main(void)
{
    ...
    while (1)
    {
        ...
        convert_entier(hum);
        convert_dizaine(hum);
        uart_printstr(" C Temperature: ");
        convert_entier(temp);
        convert_dizaine(temp);
        uart_printstr(" %\n\r");
